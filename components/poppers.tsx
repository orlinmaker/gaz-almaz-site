              ...
              >
                <div className="flex flex-col items-center text-center">
                  <div className="relative w-full aspect-[16/9] mb-6 rounded-xl overflow-hidden group-hover:scale-105 transition-transform duration-500">
                    <Image
                      src={popper.image || "/placeholder.svg"}
                      alt={popper.name}
                      fill
                  ...
                  <h3 className="text-2xl font-bold mb-3 text-foreground">{popper.name}</h3>
                  <p className="text-muted-foreground mb-4 leading-relaxed">{popper.description}</p>
                  <p className="text-xl font-semibold text-primary mb-6">{popper.priceLabel}</p>
                  ...
                  <div className="relative z-20 flex flex-col sm:flex-row gap-3 w-full">
                    <Button
                      ...
                      asChild
                    >
                      <Link href={`/poppers/${popper.id}`}>Подробнее</Link>
                    </Button>
                    <Button
                    ...
                    >
                      <a
                        href={`https://wa.me/79990000000?text=${encodeURIComponent(
                          `Здравствуйте! Хочу заказать: ${popper.name}`,
                        )}`}
                        target="_blank"
