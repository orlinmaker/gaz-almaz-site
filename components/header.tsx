...
export function Header() {
   ...
   * - если ссылка вида "/#section" и мы НЕ на главной → даём обычную навигацию (без preventDefault) на "/#section"
   * - если мы на главной → плавно скроллим к элементу id="section"
   * - любые другие href (обычные страницы) не трогаем
   */
  const handleNavClick = (e: React.MouseEvent<HTMLAnchorElement>, href: string) => {
    if (!href.startsWith("/#")) return // обычные маршруты (например, "/products")

    const id = href.split("#")[1] || ""
    if (!id) return

    // если мы не на главной — позволяем перейти на /#id (без preventDefault)
